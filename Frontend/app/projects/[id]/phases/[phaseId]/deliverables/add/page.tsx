...
const AddDeliverablePage = () => {
    ...
    const fetchProjectAndPhase = async () => {
      ...
      try {
        ...
        if (!projectResponse.success || !projectResponse.data) {
          console.error("Failed to fetch project or no data found");
          return;
        }
        setProject(projectResponse.data as Project);

        const phaseResponse = await deliverablePhaseApi.getDeliverablePhaseById(
          phaseId
        );
        if (!phaseResponse.success || !phaseResponse.data) {
          console.error("Failed to fetch phase or no data found");
          return;
        }
        setPhase(phaseResponse.data as DeliverablePhase);
      } catch (error) {
        console.error("Error fetching project or phase:", error);
      }
    };
  ...
  useEffect(() => {
    const fetchTeamMembers = async () => {
      ...
      try {
        ...
        const project = response.data as { members: string[] };
        if (!Array.isArray(project.members)) {
          console.error("Project members are not an array");
          return;
        }

        const userFetches = await Promise.all(
          project.members.map((id: string) => api.getUserById(id))
        );

        const users = userFetches
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const responseA = await projectApi.getProjectById(projectId);
    if (!responseA.success || !responseA.data) {
      console.error("Failed to fetch project or no data found");
      return;
    }
    const project = responseA.data as { clientId: string };
    ...
    if (response.success) {
      ...
      console.error("Error creating deliverable:", response.message);
    }
  };
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                      {teamMembers.map((member) => (
                        ...
                        >
                          <div
                            className="h-10 w-10 rounded-full flex items-center justify-center from-[#27acaa] to-[#32c5c2] text-white mr-3 shadow-sm"
