...
const EditTaskPage = () => {
    ...
    const fetchTask = async () => {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          setSelectedProjectId(taskData.projectId);
          setSelectedDeliverableId(taskData.deliverableId || null);
          setSelectedPhaseId(taskData.deliverablePhaseId || null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const errors = validateTaskForm(updatedTask);
  ...
  useEffect(() => {
    if (selectedProjectId && phases.length > 0) {
      // When a project is selected or changed, update the filtered phases
      const projectPhases = phases.filter(
        (phase) => phase.projectId === selectedProjectId
      );

      // If the current selected phase doesn't belong to this project, reset it
      if (
        selectedPhaseId &&
        !projectPhases.some((phase) => phase.id === selectedPhaseId)
      ) {
        setSelectedPhaseId(null);
        setSelectedDeliverableId(null);
      }
    }
  }, [selectedProjectId, phases, selectedPhaseId]);
  ...
  useEffect(() => {
    if (selectedPhaseId) {
      ...
      // If the currently selected deliverable exists but isn't in the filtered list,
      // we need to find it and add it to ensure it remains selectable
      if (
        selectedDeliverableId &&
        !filtered.some((d) => d.id === selectedDeliverableId)
      ) {
        const currentDeliverable = deliverables.find(
          (d) => d.id === selectedDeliverableId
        );
        if (currentDeliverable) {
          filtered.push(currentDeliverable);
        }
      }

      setFilteredDeliverables(filtered);
    } else {
      // If no phase is selected, show all deliverables
      setFilteredDeliverables(deliverables);
    }
  }, [selectedPhaseId, deliverables, selectedDeliverableId]);
...
export default withAuthAdminPM(EditTaskPage);
